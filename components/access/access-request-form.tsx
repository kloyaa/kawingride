...
export function AccessRequestForm() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[1.7rem] border border-slate-100 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-950/60 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
              Follow the request from details to verification.
            </p>
          </div>
          <span className="rounded-full bg-white px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950 dark:text-slate-300 dark:ring-slate-700">
            Step {String(currentStep).padStart(2, "0")} of 03
          </span>
        </div>

        <div className="mt-4 grid gap-2.5 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto_minmax(0,1fr)] sm:items-center">
          {steps.map((step, index) => {
            const stepNumber = index + 1;
            const isActive = currentStep === stepNumber;
            const isComplete = currentStep > stepNumber;
            const isSuccessStep = Boolean(successMessage) && stepNumber === 3;

            return (
              <div key={step.title} className="contents">
                <div
                  className={[
                    "relative rounded-[1.2rem] px-3 py-3 transition sm:min-h-[5.5rem]",
                    isSuccessStep
                      ? "bg-emerald-50/90 dark:bg-emerald-500/10"
                      : isActive
                        ? "bg-brand-50/80 dark:bg-brand-500/10"
                        : isComplete
                          ? "bg-emerald-50/85 dark:bg-emerald-500/10"
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={[
                        "inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl font-display text-xs font-bold",
                        isSuccessStep
                    ...
                    <div className="min-w-0">
                      <h3 className="truncate font-display text-sm font-bold text-slate-950 dark:text-white sm:text-base">
                        {step.title}
                      </h3>
                ...
                {index < steps.length - 1 ? (
                  <div className="hidden sm:flex sm:justify-center">
                    <span className="flow-dotted-link h-px w-5" />
                  </div>
                ) : null}
              </div>
            );
